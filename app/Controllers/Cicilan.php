...
private function getAllPinjamanAktif($id_anggota)
{
    ...
    try {
        log_message('debug', '=== MENGAMBIL PINJAMAN AKTIF DARI 3 TABEL ===');
        ...
        foreach ($qard_aktif as $qard) {
            log_message('debug', 'Qard Detail - ID: ' . $qard->id_qard . 
                                ', Pinjam: ' . $qard->jml_pinjam . 
                                ', Terbayar: ' . $qard->jml_terbayar . 
                                ', Tenor: ' . $qard->jml_angsuran);
            
            // PERBAIKAN: Hitung angsuran berjalan yang benar
            ...
            $pinjaman_data = [
                ...
                'bisa_bayar' => $angsuran_berjalan < $qard->jml_angsuran && $qard->jml_angsuran > 0,
                'angsuran_selanjutnya' => $angsuran_berjalan + 1 // PERBAIKAN: Tambah field ini
            ];
            
            log_message('debug', 'Qard Data - Angsuran Berjalan: ' . $angsuran_berjalan . 
                                ', Angsuran per Bulan: ' . $angsuran_per_bulan . 
                                ', Bisa Bayar: ' . ($pinjaman_data['bisa_bayar'] ? 'Ya' : 'Tidak'));
        ...
        foreach ($murabahah_aktif as $murabahah) {
            ...
            $pinjaman_data = [
                ...
                'bisa_bayar' => $angsuran_berjalan < $murabahah->jml_angsuran && $murabahah->jml_angsuran > 0,
        ...
        foreach ($mudharabah_aktif as $mudharabah) {
            ...
            $pinjaman_data = [
                ...
                'bisa_bayar' => $angsuran_berjalan < $mudharabah->jml_angsuran && $mudharabah->jml_angsuran > 0,
                'angsuran_selanjutnya' => $angsuran_berjalan + 1
            ];
...
private function hitungAngsuranBerjalanFix($total_terbayar, $total_pinjaman, $tenor)
{
    ...
    private function getSummaryCicilan($pinjaman_aktif)
    {
        ...
        foreach ($pinjaman_aktif as $pinjaman) {
            ...
            if ($pinjaman->bisa_bayar) {
                $today = date('Y-m-d');
                if (strtotime($pinjaman->jatuh_tempo_berikutnya) >= strtotime($today)) {
                    if (!$jatuh_tempo_terdekat || $pinjaman->jatuh_tempo_berikutnya < $jatuh_tempo_terdekat) {
                        $jatuh_tempo_terdekat = $pinjaman->jatuh_tempo_berikutnya;
                    }
                }
            }
        }
